    ...
    <>
      ...
      <section className="bg-background w-full py-12 md:py-20">
        <div className="max-w-4xl mx-auto px-6 flex flex-col items-center text-center">
          ...
          <h1 className="text-3xl md:text-5xl font-bold tracking-tight text-foreground mt-6">
            {barbershop.name}
          </h1>
      ...
      <section className="bg-background py-16">
        <div className="max-w-5xl mx-auto px-6">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Confira o que oferecemos</p>
          </div>

          {barbershop.services.length === 0 ? (
            <p className="text-muted-foreground mt-8">Nenhum serviço cadastrado ainda.</p>
          ) : (
      ...
      {barbershop.barbers.length > 0 && (
        <section className="bg-background border-t border-border/50 py-16">
          <div className="max-w-5xl mx-auto px-6">
            <h2 className="text-2xl font-bold text-foreground">Nossa equipe</h2>
              ...
              {barbershop.barbers.map((barber) => {
                ...
                return (
                  <div key={barber.id} className="flex flex-col items-center">
                    ...
                    <p className="text-base font-semibold text-foreground mt-3 text-center">
                      {barber.name}
                    </p>
                    {barber.bio && (
                      <p className="text-xs text-muted-foreground line-clamp-2 mt-1 text-center">
                        {barber.bio}
                      </p>
                    )}
                  </div>
      ...
      {barbershop.businessHours.length > 0 && (
        <section className="bg-background border-t border-border/50 py-16">
          <div className="max-w-2xl mx-auto px-6">
            ...
            <div className="mt-8 bg-card border border-border rounded-xl divide-y divide-zinc-800">
              {barbershop.businessHours.map((bh) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`text-sm ${
                        isToday ? "text-foreground font-medium" : "text-foreground/80"
                      }`}
                    >
